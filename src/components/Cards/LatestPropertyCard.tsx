...
function LatestPropertyCard({
  ...
  const { mutate } = useMutation({
    mutationFn: (propertyId: number) =>
      wishlistApi.add(Number(user.id), propertyId),
    onSuccess: () => {
        ...
        <div className="homec-property__head ">
          ...
          {user?.id && (
            <div className="homec-property__hsticky">
              <a className="homec-heart" onClick={() => mutate(id)}>
                <svg
                  width="23"
                  height="20"
                  viewBox="0 0 23 20"
                  fill="none"
                  color={isMyWishlist ? "red" : ""}
